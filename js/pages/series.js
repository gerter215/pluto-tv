...
const PageSeries = (() => {
  ...
  async function loadMore() {
    ...
    try {
      const items = await API.getSeries(currentPage, currentGenreId, currentFilter);
      if (Array.isArray(items)) {
        renderGrid(items, true);
      }
      currentPage++;
    } catch (err) {
      console.error('[Series] loadMore:', err);
  ...
  function renderGrid(items, append = false) {
    ...
    for (const item of items) {
      if (item.image || item.cover) {
        grid.appendChild(UI.createPosterCard(item, (i) => Detail.openSeries(i)));
      }
    }
  ...
  async function render(container, state) {
    ...
    header.innerHTML = `
      <h1 class="page-title">سریال‌ها</h1>
      <p class="page-subtitle">مجموعه سریال‌ها</p>
    `;
    ...
    try {
      ...
      if (Array.isArray(items) && items.length > 0) {
        ...
        container.appendChild(UI.createEmptyState('سریالی یافت نشد'));
      }
    } catch (err) {
      console.error('[Series]', err);
      container.appendChild(UI.createEmptyState('خطا در بارگذاری سریال‌ها'));
    } finally {
      UI.hideLoading();
    }
  }
